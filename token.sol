...
contract Megalodon is IBEP20, Auth {
    ...
    function changeBuyTax(uint256 _liquidityFee, uint256 _marketingFee, uint256 _projectFee, uint256 _reflectionFee, uint256 _burnFee) external onlyOwner {
        liquidityBuyFee = _liquidityFee;
        marketingBuyFee = _marketingFee;
        projectBuyFee = _projectFee;
        reflectionBuyFee = _reflectionFee;
        buyfeeburn = _burnFee;
        totalBuyFee = _liquidityFee.add(_marketingFee).add(_projectFee).add(_reflectionFee).add(_burnFee);
    }

    function changeSellTax(uint256 _liquidityFee, uint256 _marketingFee, uint256 _projectFee, uint256 _reflectionFee, uint256 _burnFee) external onlyOwner {
        liquiditySellFee = _liquidityFee;
        marketingSellFee = _marketingFee;
        projectSellFee = _projectFee;
        reflectionSellFee = _reflectionFee;
        sellfeeburn = _burnFee;
        totalSellFee = _liquidityFee.add(_marketingFee).add(_projectFee).add(_reflectionFee).add(_burnFee);
    }
